...
const MeetingLogger = ({ groupId }: { groupId: string }) => {
    ...
    return (
        <section className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                <UsersIcon size={80} />
            </div>

            <div className="mb-10 relative">
                <h3 className="text-2xl font-black text-white mb-2 ml-1">Log Real Reunion</h3>
                <p className="text-slate-500 font-medium ml-1">Storing your memories permanently in Supabase.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {availableUsers.length > 0 ? availableUsers.map(user => (
                            <button
                                ...
                                className={`flex items-center gap-3 px-4 py-3 rounded-2xl border-2 transition-all ${selectedMembers.includes(user.id)
                            ...
                            >
                                <div className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center font-bold text-[10px]">
                                    {selectedMembers.includes(user.id) ? <Check size={12} /> : (user?.name?.charAt(0) || '?')}
                                </div>
                                <span className="text-xs font-bold">{user?.name || 'Unknown'}</span>
                            </button>
                        )) : (
                            <p className="text-slate-600 text-xs italic p-4 bg-slate-950 w-full rounded-2xl border border-slate-800">
                ...
                <div>
                    ...
                    <div className="flex items-start gap-6">
                        <label className="w-32 h-32 bg-slate-950 border-2 border-dashed border-slate-800 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 hover:bg-slate-900 transition-all group shrink-0">
                        ...
                        {mediaPreview && (
                            <div className="relative w-full h-32 bg-slate-950 rounded-3xl overflow-hidden border border-slate-800">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className="text-[10px] font-black text-blue-500 bg-blue-500/10 px-4 py-2 rounded-full border border-blue-500/20 uppercase tracking-[0.2em] backdrop-blur-md">File Ready to Sync</span>
                                </div>
                                <button
                                    type="button"
                ...
                <div className="flex justify-end pt-8 border-t border-slate-800">
                    <button
                        ...
                        className="bg-white text-black font-black uppercase tracking-[0.15em] px-12 py-5 rounded-2xl hover:scale-[1.03] active:scale-[0.97] transition-all disabled:opacity-30 disabled:hover:scale-100 flex items-center gap-3"
                    >
